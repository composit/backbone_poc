/* Call barChart() to create a d3 bar chart from timeseries data. 
 * Example: barChart().(d3.select('#memory_chart').datum(data))
 * data can be one of the following formats:
 * - a Hash w/key = series name and the values a 2D array of times and y values.
 * - a 2D array of times and values
...
function BarChart(params) {
  ...
  var color = d3.scale.ordinal()
    .range(colors)
    .domain(d3.range(0, (this.data.length ? this.data[0].value.length : 0)));
